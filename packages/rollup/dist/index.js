...
function transformConfig(code, isModule = false) {
    return core.transformSync(code, { module: { type: isModule ? "es6" : "commonjs" }, jsc: { parser: { syntax: "typescript" }, target: "esnext" }, swcrc: false, configFile: false }).code;
}
...
async function loadConfigFile() {
    ...
    const jsConfigFile = path$2.join(cwd, "macros.config.js");
    if (fs.existsSync(jsConfigFile)) {
        // {type: "module" | "commonjs"} & "macros.config.js"
        const config = await autoRequire(jsConfigFile);
        return [jsConfigFile, config];
    }
    const tsConfigFile = path$2.join(cwd, "macros.config.ts");
    if (fs.existsSync(tsConfigFile)) {
        // {type: "module"} & "macros.config.ts"
        if (isModule()) {
            const jsConfigFile = path$2.join(cwd, ".macros.config.js");
            const code = fs.readFileSync(tsConfigFile).toString();
            const transformed = transformConfig(code, true);
            await fs.writeFile(jsConfigFile, transformed, (err) => {
                if (err != null)
                    throw new Error(err.message);
            });
            const config = (await import(jsConfigFile) || {}).default;
            fs.rm(jsConfigFile, (err) => {
                if (err != null)
                    throw new Error(err.message);
            });
...
function applySwcOptions(config) {
    var _a;
    const swcOptions = extractSwcOptions(config);
    if (!((_a = swcOptions.jsc) === null || _a === void 0 ? void 0 : _a.target)) {
        set(swcOptions, "jsc.target", 
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        nodeTargetDefaults.get(process$1.version.match(/v(\d+)/)[1]) || "es2018");
    }
...
function applyMacroOptions(config) {
    if (config.emitDts && !config.onEmitDts) {
        config.onEmitDts = (dts) => writeDts(path$2.resolve(config.dtsOutputPath || "./macros.d.ts"), dts);
    }
    return config;
}
...
const isExternal = (id) => /(@macro-plugin)|@swc/.test(id);
async function rollupMacroPlugin(config) {
    ...
    const plugin = {
        ...
        async transform(code, id) {
            ...
            if (id.endsWith(".js")) {
                const result = await core$1.transformAsync(code, macroOptions);
                return {
                    code: result.code,
                    map: result.map
                };
            ...
            if (isTs || isJsx) {
                const plugin = core$1.createSwcPlugin(macroOptions, code, core$1.getSpanOffset());
                const options = Object.assign(Object.assign({}, computedSwcOptions), { module: Object.assign(Object.assign({}, computedSwcOptions.module), { 
                        // async transform is always ESM
                        type: "es6" }), plugin, filename: id });
                const parser = isTs
                    ? {
                        syntax: "typescript",
                        tsx: isJsx,
                ...
                const result = await core.transform(code, options);
                return {
                    code: result.code,
                    map: result.map
                };
        ...
        options(options) {
            ...
            options.onwarn = (warning, warn) => {
                if (warning.exporter && isExternal(warning.exporter) && ["UNRESOLVED_IMPORT", "UNUSED_EXTERNAL_IMPORT"].includes(warning.code || ""))
                    return;
            ...
            if (typeof oldTreeshake === "object") {
                options.treeshake = Object.assign(Object.assign({}, oldTreeshake), { moduleSideEffects(id, external) {
                        if (isExternal(id))
                            return false;
                ...
                options.treeshake = {
                    moduleSideEffects(id) {
                        if (isExternal(id))
                            return false;
                        if (typeof oldTreeshake === "boolean")
                            return oldTreeshake;
                        return true;
                    },
...
module.exports = rollupMacroPlugin;
