import { isMacroPlugin } from '@macro-plugin/core';
import B, { promises, constants as constants$3, existsSync, readFile, readFileSync, writeFile, rm, statSync, readdirSync } from 'fs';
import p, { resolve, isAbsolute, win32, posix, normalize, dirname, join, extname } from 'path';
import cn, { createRequire } from 'module';
...
const fileExists = (path) => {
    return promises.access(path, constants$3.F_OK)
        .then(() => true)
        .catch(() => false);
};
const importLib = (moduleId, dir = CWD) => createRequire(resolve(dir, "noop.js"))(moduleId);
...
function resolveMacroOptions(config) {
    if (config.emitDts && !config.onEmitDts) {
        config.onEmitDts = (dts) => writeDts(resolve(config.dtsOutputPath || "./macros.d.ts"), dts);
    }
    if (config.depends) {
        if (!config.macros)
            config.macros = [];
