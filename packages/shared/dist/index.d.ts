...
import { CompilerOptions, ResolvedProjectReference, ResolutionMode } from 'typescript';
...
declare const SCRIPT_EXTENSIONS: string[];
 ...
 */
declare const resolveTsOptions: (cwd: string, tsconfig?: string) => CompilerOptions;
declare function resolveSwcOptions(config: Config): Options$1;
declare function patchTsOptions(options: Options$1, tsOptions: CompilerOptions | undefined, isTypeScript: boolean, isTsx: boolean, isJsx: boolean): Options$1;
...
declare function createTsLib(): TsLib;
declare function normalizePath(filename: string): string;
declare function createModuleResolver(ts: typeof typescript, host: FormattingHost): (moduleName: string, containingFile: string, redirectedReference?: ResolvedProjectReference, mode?: ResolutionMode) => typescript.ResolvedModuleFull | undefined;
declare function createFormattingHost(ts: typeof typescript, compilerOptions: CompilerOptions): FormattingHost;
/** create a rollup/vite typescript file resolver */
declare function createTsResolver(tslib: TsLib, compilerOptions: CompilerOptions, filter: Filter): (importee: string, importer: string | undefined) => string | null;
/** create a rollup/vite file resolver */
declare function createResolver(extensions: string[]): (importee: string, importer: string | undefined) => Promise<string | null | undefined>;
...
declare const DEFAULT_INCLUDE: RegExp[];
declare const DEFAULT_EXCLUDE: RegExp[];
declare const createFilter: (include?: (string | RegExp)[], exclude?: (string | RegExp)[]) => (id: string) => boolean;
 ...
 */
declare function ensureArray<T>(items: (T | false | null | undefined)[] | T | false | null | undefined): T[];
/**
 * Turn `config.external` into a function, for rollup and vite plugin usage.
 * @param external config.external from rollup config
 * @returns
 */
declare const getIdMatcher: <T extends any[]>(external: string | boolean | RegExp | (string | RegExp)[] | ((id: string, ...parameters: T) => boolean | null | void) | undefined) => (id: string, ...parameters: T) => boolean;
declare const getHasModuleSideEffects: (moduleSideEffectsOption: boolean | "no-external" | string[] | HasModuleSideEffects | undefined) => (_id: string, external: boolean) => boolean;
declare const matchScriptType: (id: string, extensions: string[]) => {
    isTypeScript: boolean;
    isTsx: boolean;
    isJsx: boolean;
} | null;

export { CWD, DEFAULT_EXCLUDE, DEFAULT_INCLUDE, Filter, FormattingHost, HasModuleSideEffects, MatchOptions, SCRIPT_EXTENSIONS, TsLib, addHook, autoRequire, buildTransformOptions, buildTransformOptionsSync, createFilter, createFormattingHost, createModuleResolver, createResolver, createTsLib, createTsResolver, ensureArray, extractFiles, extractInput, extractSwcOptions, fileExists, getHasModuleSideEffects, getIdMatcher, hasProp, hookRequire, importLib, isModule, loadConfigFile, loadConfigFileSync, matchPattern, matchScriptType, normalizePath, patchTsOptions, picomatch, resolveDepends, resolveExternals, resolveMacroOptions, resolveSwcOptions, resolveTsOptions, transformConfig, writeDts };
