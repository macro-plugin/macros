...
const fixCallee = (callee) => {
    ...
    if (callee.type !== "Identifier")
        return { type: "ParenthesisExpression", span, expression: callee };
    return callee;
};
                ...
                if (callee.type === "ArrayExpression") {
                    ...
                    els.length > 1 && els.forEach((el, index) => {
                        if (lastCallee.type === "CallExpression") {
                            ...
                            lastCallee = { type: "CallExpression", span, callee: fixCallee(el === null || el === void 0 ? void 0 : el.expression), arguments: index === last ? [{ expression }] : [] };
                            if (index === last)
                                finalCallee = callee;
                            cacheCallee.arguments[0] = { expression: lastCallee };
                        }
                        else {
                            callee = { type: "CallExpression", span, callee, arguments: [{ expression: fixCallee(el === null || el === void 0 ? void 0 : el.expression), spread: el === null || el === void 0 ? void 0 : el.spread }] };
                            lastCallee = callee;
                        }
                    });
                }
...
function _Quote(strings, ...exprs) {
    const exprMarkers = exprs.map((_, i) => "_macro_marker_" + i + "_");
    const options = { syntax: "typescript", tsx: true };
    const ast = parseSync(printRawTmpl(strings, exprMarkers), options).body;
    walkObject(ast, (k, v, parent) => {
        if (k === "span") {
            parent[k] = span;
            return;
