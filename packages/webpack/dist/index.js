    ...
    const loader = function (source, inputSourceMap) {
        ...
        const compile = () => {
            ...
            if (filename.endsWith(".js")) {
                ...
                core$1.transformAsync(source, macroOptions).then(output => {
                    callback(null, output.code, output.map);
                }, callback);
            }
            else {
                const offset = core$1.getSpanOffset();
                swcOptions.plugin = core$1.createSwcPlugin(macroOptions, source, offset);
                // using swc transform ts/js/tsx
                core.transform(source, swcOptions).then(output => {
                    callback(null, output.code, output.map);
                }, callback);
            }
        };
