...
async function loadConfigFile() {
    ...
    if (require$$1.existsSync(jsConfigFile)) {
    ...
    if (require$$1.existsSync(tsConfigFile)) {
        // {type: "module"} & "macros.config.ts"
        if (isModule()) {
            const jsConfigFile = path$2.join(cwd, ".macros.config.js");
            const code = require$$1.readFileSync(tsConfigFile).toString();
            const transformed = transformConfig(code, true);
            await require$$1.writeFile(jsConfigFile, transformed, (err) => {
                if (err != null)
                    throw new Error(err.message);
            });
            const config = (await import(jsConfigFile) || {}).default;
            require$$1.rm(jsConfigFile, (err) => {
                if (err != null)
                    throw new Error(err.message);
            });
...
function writeDts(p, dts) {
    const emit = () => require$$1.writeFile(p, dts, () => { });
    if (require$$1.existsSync(p)) {
        require$$1.readFile(p, (err, data) => {
...
async function createTransformer(inputOptions) {
    ...
    return {
        ...
        process(src, filename, jestOptions) {
            ...
            return core.transformSync(src, Object.assign(Object.assign({}, computedSwcOptions), { module: Object.assign(Object.assign({}, computedSwcOptions.module), { type: (jestOptions.supportsStaticESM ? "es6" : "commonjs") }), plugin: core$1.createSwcPlugin(macroOptions, src, offset), filename }));
        },
        async processAsync(src, filename, jestOptions) {
            insertInstrumentOptions(jestOptions, !!canInstrument, computedSwcOptions, instrumentOptions);
            const offset = core$1.getSpanOffset();
            return await core.transform(src, Object.assign(Object.assign({}, computedSwcOptions), { module: Object.assign(Object.assign({}, computedSwcOptions.module), { 
                    // async transform is always ESM
                    type: "es6" }), plugin: core$1.createSwcPlugin(macroOptions, src, offset), filename }));
        },
        getCacheKey(src, filename, ...rest) {
            ...
            return require$$0$1.createHash("md5")
                .update(baseCacheKey)
                .update("\0", "utf8")
